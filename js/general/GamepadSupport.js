...
function GamepadSupport() {
  ...
  var SIMULATED_KEY_UP = 38; //up //87;//w
  ...
  if (!navigator.getGamepads) {
    console.log(
      "Your browser does not allow the use of gamepads. Please change your browser settings if you want to use a gamepad."
    );
  } else {
    console.log(
      "Gamepad Support is ENABLED. Click any button on your gamepad to connect!"
    );
  }
  ...
  this.handle_gamepad = function () {
    if (!gamepad) {
      // always null until you press a button!
    ...
    if (gamepad) {
      ...
      // Remapping direction controls to d-pad
  ...
  this.fake_keyboard_events = function () // if any
  {
    ...
    if (!prev_gamepad_left && gamepad_left)
      this.simulateKeyDown(SIMULATED_KEY_LEFT);
    if (!prev_gamepad_right && gamepad_right)
      this.simulateKeyDown(SIMULATED_KEY_RIGHT);
    if (!prev_gamepad_up && gamepad_up) this.simulateKeyDown(SIMULATED_KEY_UP);
    if (!prev_gamepad_down && gamepad_down)
      this.simulateKeyDown(SIMULATED_KEY_DOWN);
    if (!prev_gamepad_fire && gamepad_b)
      this.simulateKeyDown(SIMULATED_KEY_B_BUTTON);
    if (!prev_gamepad_jump && gamepad_a)
      this.simulateKeyDown(SIMULATED_KEY_A_BUTTON);
    if (!prev_gamepad_x && gamepad_x)
    ...
    if (prev_gamepad_right && !gamepad_right)
      this.simulateKeyUp(SIMULATED_KEY_RIGHT);
    if (prev_gamepad_up && !gamepad_up) this.simulateKeyUp(SIMULATED_KEY_UP);
    if (prev_gamepad_down && !gamepad_down)
      this.simulateKeyUp(SIMULATED_KEY_DOWN);
    if (prev_gamepad_fire && !gamepad_b)
      this.simulateKeyUp(SIMULATED_KEY_B_BUTTON);
    if (prev_gamepad_jump && !gamepad_a)
      this.simulateKeyUp(SIMULATED_KEY_A_BUTTON);
    if (prev_gamepad_x && !gamepad_x)
